...
class UQ_SNGP(tf.keras.Model):
    def __init__(self, model_fn, input_shape=(16,), gamma=0.5, optimizer='adam',
                 learning_rate=0.001, loss_fn='mse', metrics_fn='mae', dynamic_weights=True, model_name='PGNN'):
        super().__init__()

        self.model = model_fn(input_shape=input_shape,gamma=gamma)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            with tf.GradientTape() as phys_tape:
                ...
                mu_phyx, cov_phyx = self.model(X_physics_batch, training=True)
            ...
            physics_loss = self.loss_fn(self.state_variable[-1], [dD_dt, cov_phyx])
    ...
    def test_step(self, data):
        ...
        X_batch = tf.cast(X_batch, tf.float32)
        y_true = tf.cast(y_true, tf.float32)
        mu,cov = self.model(X_batch, training=False)
        return {
            "loss": self.loss_fn(y_true, [mu, cov]),
    ...
    def predict(self, X):
        ...
        mu, var = self.model(X, training=False)
        mu = mu.numpy().squeeze()
        var = tf.linalg.diag_part(var).numpy().squeeze()
        return mu, var
