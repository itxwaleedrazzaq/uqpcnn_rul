...
class UQ_MC(tf.keras.Model):
    def __init__(self, model_fn, dropout_rate, input_shape=(16,), optimizer='adam',
                 learning_rate=0.001, loss_fn='mse', metrics_fn='mae', dynamic_weights=True, model_name='UQ_MC',**model_kwargs):
        super().__init__()

        self.model = model_fn(input_shape=input_shape, dropout_rate=dropout_rate, **model_kwargs)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            with tf.GradientTape() as phys_tape:
                ...
                mu_phyx = self.model(X_physics_batch, training=True)
            
            # Calculate time derivative
            dD_dt = phys_tape.gradient(mu_phyx, X_physics_batch)[:, 14:15]
            ...
            physics_loss = self.loss_fn(self.state_variable[-1], dD_dt)
    ...
    def test_step(self, data):
        ...
        X_batch = tf.cast(X_batch, tf.float32)
        y_true = tf.cast(y_true, tf.float32)
        mu = self.model(X_batch, training=False)
        return {
            "loss": self.loss_fn(y_true, mu),
    ...
    def _predict(self, X, num_samples=50):
        X = tf.cast(X, tf.float32)
        preds = [self.model(X, training=False).numpy() for _ in range(num_samples)]
        all_samples = np.stack(preds, axis=0)
        mean_pred = np.mean(all_samples, axis=0)
        var_pred = np.var(all_samples, axis=0)
        return mean_pred.squeeze(), var_pred.squeeze()